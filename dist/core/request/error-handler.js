...
export class ErrorHandler {
    ...
    async handle(error, response, account, context, showToast) {
        ...
        if (response.status === 401 && context.retry < this.config.rate_limit_max_retries) {
            const reason = await readBody();
            logger.warn(`HTTP 401 on ${account.email} (retry ${context.retry}): ${reason || 'Unauthorized'}`);
        ...
        if (response.status === 500) {
            ...
            if (account.failCount < 5) {
                ...
                this.accountManager.markUnhealthy(account, `Server error (500) after 5 attempts: ${errorMessage}`, Date.now() + 1800000 // 30 min recovery
                );
