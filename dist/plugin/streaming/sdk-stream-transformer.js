...
export async function* transformSdkStream(sdkResponse, model, conversationId, toolNameMapper, thinkingRequested = false) {
    const streamState = {
        thinkingRequested,
        buffer: '',
        inThinking: false,
    ...
    try {
        for await (const event of eventStream) {
            if (event.assistantResponseEvent?.content) {
                ...
                if (event.metadataEvent.tokenUsage) {
                    ...
                    if (typeof tu.inputTokens === 'number')
                        realInputTokens = tu.inputTokens;
                    if (typeof tu.outputTokens === 'number')
                        realOutputTokens = tu.outputTokens;
                }
            }
                ...
                logger.debug(`[CREDITS] usage=${me.usage} ${me.unit || 'credit'}${me.usage !== 1 ? 's' : ''}`);
            }
        }
        if (activeToolCalls.size > 0) {
            ...
            for (const truncated of activeToolCalls.values()) {
                logger.debug(`[STREAM] Truncated tool call: name=${truncated.name} id=${truncated.toolUseId} inputLen=${truncated.input.length}`);
                for (const ev of createTextDeltaEvents(`\n\n[Kiro: "${truncated.name}" truncated mid-stream — 64K token output limit exceeded. Write in chunks of ≤500 lines.]`, streamState)) {
                    const _c = convertToOpenAI(ev, conversationId, model);
                    if (_c !== null)
                        yield _c;
                }
        ...
        logger.debug(`[STREAM] Error in transformSdkStream: ${e instanceof Error ? e.message : String(e)}`);
        for (const tc of activeToolCalls.values()) {
            logger.debug(`[STREAM] Incomplete tool call: name=${tc.name} id=${tc.toolUseId} inputLen=${tc.input.length}`);
        }
        throw e;
    }
}
