 ...
 */
export function resolveKiroEndpoint(auth) {
    const region = auth.region || 'us-east-1';
    if (auth.profileArn) {
        return buildUrl(KIRO_CONSTANTS.RUNTIME_URL, region);
    }
    return buildUrl(KIRO_CONSTANTS.BASE_URL, region);
}
