...
import { buildToolNameMaps, convertToolsToCodeWhisperer, deduplicateToolResults, shortenToolName } from '../infrastructure/transformers/tool-transformer.js';
import { getEffectiveEffort } from './effort.js';
import { imageCache } from './image-cache.js';
import { MAX_KIRO_IMAGES, convertImagesToKiroFormat, extractAllImages, extractTextFromParts } from './image-handler.js';
...
function deriveConversationIds(workspace, firstUserContent) {
    ...
    if (existing) {
        if (!existing.agentContinuationId) {
            ...
            kiroDb.setConversationId(workspace, fingerprint, existing.convId, existing.agentContinuationId);
        }
...
function buildCodeWhispererRequest(body, model, auth, think = false, budget = 20000, showToast, workspace = '', carryForward = true, sessionId, maxPayloadBytes = 4_000_000) {
    ...
    const { convId, agentContinuationId, fingerprint } = deriveConversationIds(workspaceKey, firstUserContent);
    ...
    if (history.length > 2) {
        ...
        const baseRequest = { ...request, conversationState: { ...request.conversationState } };
        ...
        while (history.length > 2 && totalSize > MAX_PAYLOAD_BYTES) {
            ...
            const toolUseIds = new Set(history.flatMap((h) => h.assistantResponseMessage?.toolUses?.map((tu) => tu.toolUseId) ?? []));
            while (history.length > 0) {
                const trs = history[0]?.userInputMessage?.userInputMessageContext?.toolResults;
                if (!trs)
                    break;
                const allMatched = trs.every((tr) => toolUseIds.has(tr.toolUseId));
                if (allMatched)
                    break;
                totalSize -= sizes.shift() || 0;
                history.shift();
    ...
    if (trimStartLen !== history.length) {
        logger.debug(`[TRIM] history ${trimStartLen}→${history.length} entries, ~${Math.round(trimSizeBefore / 1024)}KB exceeded ${Math.round(MAX_PAYLOAD_BYTES / 1024)}KB cap`);
    }
    ...
    if (uim?.userInputMessageContext?.toolResults) {
        const survivingToolUseIds = new Set(history.flatMap((h) => h.assistantResponseMessage?.toolUses?.map((tu) => tu.toolUseId) ?? []));
        const filtered = uim.userInputMessageContext.toolResults.filter((tr) => survivingToolUseIds.has(tr.toolUseId));
    ...
    if (carryForward && uim) {
        ...
        const wireImages = cmImgs.length > 0 ? [...cmImgs] : [];
        if (wireImages.length < MAX_KIRO_IMAGES &&
            imageCache.hasEverHadImages(workspaceKey, fingerprint)) {
            for (const h of history) {
                const imgs = h.userInputMessage?.images;
...
export function transformToCodeWhisperer(url, body, model, auth, think = false, budget = 20000, workspace = '', carryForward = true, sessionId) {
    const { request, resolved, convId } = buildCodeWhispererRequest(body, model, auth, think, budget, undefined, workspace, carryForward, sessionId);
...
export function transformToSdkRequest(body, model, auth, think = false, budget = 20000, showToast, workspace = '', carryForward = true, sessionId, effortConfig, maxPayloadBytes = 4_000_000) {
    const { request, resolved, convId, fingerprint, toolNameMapper } = buildCodeWhispererRequest(body, model, auth, think, budget, showToast, workspace, carryForward, sessionId, maxPayloadBytes);
    ...
    if (process.env.DEBUG || process.env.OPENCODE_LOG_LEVEL === 'debug') {
        ...
        logger.debug(`[PAYLOAD] convId=${convId} ~${Math.round(finalBytes / 1024)}KB history=${request.conversationState.history?.length ?? 0} entries`);
    }
    return {
