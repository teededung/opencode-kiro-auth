...
import { MAX_KIRO_IMAGES, MAX_KIRO_IMAGE_BYTES } from './image-handler.js';
...
function dedupKey(img) {
    const b = img.source.bytes;
    const n = b.byteLength;
    if (n === 0)
        return `${img.format}:empty`;
    const hex = (start, end) => Array.from(b.subarray(start, end), (v) => v.toString(16).padStart(2, '0')).join('');
...
function diskFileFor(cacheDir, workspace, fingerprint) {
...
export class ImageCache {
    ...
    constructor(opts = {}) {
        ...
        if (this.cacheDir)
            this.sweepDiskExpired();
    }
    k(workspace, fingerprint) {
        return `${workspace}\0${fingerprint}`;
    }
    set(workspace, fingerprint, images) {
        if (images.length === 0)
            return;
     ...
     */
    upsert(workspace, fingerprint, newImages) {
        const key = this.k(workspace, fingerprint);
        ...
        existing ??= [];
        if (newImages.length === 0 && existing.length === 0)
            return 0;
        const seen = new Set();
        const merged = [];
        const add = (img) => {
            if (merged.length >= MAX_KIRO_IMAGES)
                return;
    ...
    hasEverHadImages(workspace, fingerprint) {
        const entry = this.cache.get(this.k(workspace, fingerprint));
        if (entry)
            return entry.everHadImages;
        if (this.cacheDir) {
            try {
    ...
    writeEntryToDisk(workspace, fingerprint, entry) {
        if (!this.cacheDir)
            return;
        try {
    ...
    loadEntryFromDisk(workspace, fingerprint) {
        if (!this.cacheDir)
            return null;
        try {
            const file = diskFileFor(this.cacheDir, workspace, fingerprint);
            ...
            for (const im of data.images) {
                if (!im || typeof im.format !== 'string' || typeof im.b64 !== 'string')
                    continue;
    ...
    deleteFromDisk(workspace, fingerprint) {
        if (!this.cacheDir)
            return;
        try {
            const file = diskFileFor(this.cacheDir, workspace, fingerprint);
